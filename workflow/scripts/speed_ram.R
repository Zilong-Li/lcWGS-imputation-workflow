...
par(mfrow=c(1,2))
plot(deps, times[,1], type = "b", lwd=1.5, pch = 1, ylab = "Total Time in seconds for the chromosome", xlab = "Samples sequencing depth", ylim=c(min(times)*0.9, max(times)*1.1))
...
plot(deps, rams[,1], type = "b", lwd=1.5, pch = 1, ylab = "Maximum RAM in KBs for the chromosome", xlab = "Samples sequencing depth", ylim=c(min(rams)*0.9, max(rams)*1.1))
